#include "halley/sprite_attribute.hlsl"

Texture2D tex0 : register(t0);
SamplerState sampler0 : register(s0);

cbuffer OutlineParams : register(b1)
{
	int u_texBPP0;
};

float sampleAlpha(float2 coord0, float4 bounds) {
	if (coord0.x < bounds.x || coord0.y < bounds.y || coord0.x >= bounds.z || coord0.y >= bounds.w) {
		return 0;
	}
	const float4 col = tex0.Sample(sampler0, coord0);
	return u_texBPP0 == 1 ? (col.r > 0.5 / 255.0 ? 1.0 : 0.0) : col.a;
}

float4 main(VOut input) : SV_TARGET {
	const float threshold = 0.01;
	const int thickness = 2;
	const float4 outlineCol = input.colour;

	float2 texCoord0Grad = input.custom0.xy;

	float colCentre = sampleAlpha(input.texCoord0.xy, input.texCoord1Bounds);
	bool hasOutline = false;

	if (colCentre >= threshold) {
		//discard;
		return outlineCol;
	}

	float2 samples[12] = { float2(1, 0), float2(1, 1), float2(0, 1), float2(-1, 1), float2(-1, 0), float2(-1, -1), float2(0, -1), float2(1, -1), float2(2, 0), float2(-2, 0), float2(0, -2), float2(0, 2) };
	for (int i = 0; i < 12; ++i) {
		float col = sampleAlpha(input.texCoord0.xy + samples[i] * texCoord0Grad, input.texCoord1Bounds);
		if (col >= threshold) {
			hasOutline = true;
		}
	}

	return hasOutline ? outlineCol : float4(0, 0, 0, 0);
}
