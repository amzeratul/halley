...
namespace Halley {
	...
	public:
		...
		static std::shared_ptr<const char> inflateToSharedPtr(gsl::span<const gsl::byte> bytes, size_t& outSize, size_t maxSize);
		static unsigned char* inflateRaw(gsl::span<const gsl::byte> bytes, size_t& outSize, size_t maxSize = std::numeric_limits<size_t>::max());
	};
}
