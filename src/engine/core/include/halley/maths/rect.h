...
namespace Halley {
	...
	public:
		...
		[[nodiscard]] constexpr Rect2D findOuterRectWithAspectRatio(Vector2D<T> size) const
		{
			static_assert(std::is_floating_point_v<T>);
			return findOuterRectWithAspectRatio(size.x / size.y);
		}

		[[nodiscard]] constexpr Rect2D findOuterRectWithAspectRatio(float ar) const
		{
			static_assert(std::is_floating_point_v<T>);
			const auto w = getSize().x;
			const auto h = getSize().y;
