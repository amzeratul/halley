...
void Entity::addComponent(Component* component, int id, const char* componentName, ComponentDeleterTable& deleterTable)
{
	...
	if (liveComponents == std::numeric_limits<decltype(liveComponents)>::max()) {
		...
		throw Exception("Too many components in Entity \"" + curName + "\".", HalleyExceptions::Entity);
	}
...
{
	if (auto* networkComponent = tryGetComponent<NetworkComponent>(true)) {
        ...
        if (!networkComponent->ownerId) {
        	...
        	// This can be a legimite call if the host creates an outbound entity for one peer, for
        	// some entity created/owned by another peer...
        	if (peerId != 0 || !networkComponent->dataInterpolatorSet.isReady()) {
        		Logger::logDev("Ignore attempt to reassign ownership of " + ref.getName() + "(" + toString(entityId.value & 0xffffffff) +
