...
void RenderSnapshot::draw(const Material& material, size_t numVertices, gsl::span<const char> objectData, gsl::span<const char> vertexData, gsl::span<const IndexType> indices, PrimitiveType primitive, bool allIndicesAreQuads)
{
	getCurDrawCall().emplace_back(CommandType::Draw, static_cast<uint16_t>(drawDatas.size()));
	drawDatas.push_back(DrawData{ &material, {}, numVertices, Vector<char>(objectData.begin(), objectData.end()), Vector<char>(vertexData.begin(), vertexData.end()), Vector<IndexType>(indices.begin(), indices.end()), primitive, allIndicesAreQuads });
	finishDrawCall();
}
...
void RenderSnapshot::playDraw(Painter& painter, const DrawData& data, std::shared_ptr<const MaterialDefinition> debugMaterial) const
{
	...
	painter.executeDrawPrimitives(*material, data.numVertices, data.objectData, data.vertexData, data.indices, data.primitive, data.allIndicesAreQuads);
}
