...
void LuaStackOps::push(const ConfigNode& node)
{
	if (node.getType() == ConfigNodeType::Undefined) {
		...
		push(node.asBool());
	} else if (node.getType() == ConfigNodeType::String || node.getType() == ConfigNodeType::RawString) {
		push(node.asStringView());
...
bool LuaStackOps::eval(const String& v, const String& name, bool throwOnError)
{
	load(v, name);
...
bool LuaStackOps::eval(const Bytes& bytes, const String& name, bool throwOnError)
{
	load(bytes, name);
	return state.call(0, 1, throwOnError);
}
