...
public:
	...
	Halley::ConfigNode serialize(const Halley::EntitySerializationContext& context) const {
		...
		Halley::EntityConfigNodeSerializer<decltype(activeStates)>::serialize(activeStates, Halley::Vector<std::shared_ptr<Halley::ScriptState>>{}, context, node, componentName, "activeStates", makeMask(Type::Network));
	...
	void deserialize(const Halley::EntitySerializationContext& context, const Halley::ConfigNode& node) {
		using namespace Halley::EntitySerialization;
		Halley::EntityConfigNodeSerializer<decltype(activeStates)>::deserialize(activeStates, Halley::Vector<std::shared_ptr<Halley::ScriptState>>{}, context, node, componentName, "activeStates", makeMask(Type::Network));
