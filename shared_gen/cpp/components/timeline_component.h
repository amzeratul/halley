...
public:
	...
	void hash(const Halley::EntitySerializationContext& _context, Halley::Hash::Hasher& _hasher) const {
		...
		Halley::EntityConfigNodeSerializer<decltype(player)>::hash(_hasher, player, _context, "player", makeMask(Type::SaveData, Type::Dynamic, Type::Network));
	}

	static void sanitize(Halley::ConfigNode& _node, int _mask) {
		using namespace Halley::EntitySerialization;
		if ((_mask & makeMask(Type::Prefab)) == 0) _node.removeKey("timeline");
		if ((_mask & makeMask(Type::SaveData, Type::Dynamic, Type::Network)) == 0) _node.removeKey("player");
		if ((_mask & makeMask(Type::Prefab)) == 0) _node.removeKey("playOnStart");
	}
	...
	void serializeNetwork(const Halley::ByteSerializationContext& _context, Halley::Serializer& _serializer) const {
		Halley::ByteSerializationHelper<decltype(player)>::serialize(player, _context, _serializer, componentIndex, "player");
	}

	void deserializeNetwork(const Halley::ByteSerializationContext& _context, Halley::Deserializer& _deserializer) {
		Halley::ByteSerializationHelper<decltype(player)>::deserialize(player, _context, _deserializer, componentIndex, "player");
	}
